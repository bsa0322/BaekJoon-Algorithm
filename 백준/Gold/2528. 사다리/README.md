...
### 풀이
- 소요 시간: `10h` (여러 일에 걸쳐 풀어서 정확히 잰 시간은 아님)
- 풀이
  - 시간 지남에 따라 현재 시간의 현재 층과 다음 층의 위치 구한 후, 다음 층으로 넘어갈 수 있는지 확인하는 시뮬레이션 문제
  - 층의 위치를 매번 O(N)으로 해결하는 것이 아닌, 시간에 따라 수식으로 **한 번에** 구하는 것이 핵심임 !!
- 이슈
  - 중간에 접근은 잘 했는데... 다음으로 넘어가는 것을 확인하는 로직이 처음부터 계속 틀렸었음... 이게 문제였다는 걸 빨리 찾았다면 더 빨리 해결했을 텐데..
  - 다음으로 넘어갈 수 있는지 확인을 양 끝 지점으로 하는 경우엔 꼭 현재 층과 다음 층을 **뒤바꿔서도** 생각해야 함을 주의 !!  
