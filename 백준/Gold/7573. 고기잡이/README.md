...
- 소요 시간: `1h`
- 풀이
  - 영역 전체를 확인할 경우 시간초과로, 물고기가 존재하는 곳만 확인해서 모든 그물을 쳐보는 브루트포스 접근
  - 과정
      1. 현재 물고기를 기준으로 상, 좌, 좌상 대각선 위를 모두 검사해서 가능한 시작 점 구함 (물고기가 왼쪽 끝 ~ 오른쪽 끝 테두리 안에 들어오는 모든 경우 고려)
      2. 위에서 구한 시작 점으로부터 가능한 모든 그물 쳐보기
      3. 그물 내에 들어오는 물고기 개수 세기 (좌표값 통해서)
      4. a ~ c 번 과정 모든 물고기에 대해 반복 수행
